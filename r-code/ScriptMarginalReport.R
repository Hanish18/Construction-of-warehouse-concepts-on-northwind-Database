...
getSQLServerData <- function() {
    ...
    db_df <- sqlQuery(con, 'select top(5) dimProduct.ProductName,dimProduct.UnitsInStock,dimProduct.UnitsOnOrder,dimProduct.UnitsInStock-dimProduct.UnitsOnOrder as "MARGINAL"
from dimProduct')
    print(db_df)
    plot(db_df)
    close(con)
...
for (i in 1:(n - windowQuery)) {
    ...
    plot(data$ProductName, data$MARGINAL, type = 'h', main = 'Realtime data')
    Sys.sleep(0.5)
}